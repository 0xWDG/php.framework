...
func mycrc32(crc: UInt32, data: NSData?) -> UInt32 {
    guard let data = data else {
        return mycrc32(0, buffer: nil, length: 0)
    }
    return mycrc32(crc, buffer: UnsafePointer<UInt8>(data.bytes), length: data.length)
}
 ...
 **PHP.Framework**\
 ...
 - Returns: the crc32 polynomial
 */
public func crc32(str: String) -> UInt32 {
    let data: NSData = str.dataUsingEncoding(NSUTF8StringEncoding)!
    return mycrc32(0, data: data)
}

func mycrc32(crc: UInt32, buffer: UnsafePointer<UInt8>, length: Int) -> UInt32 {
    if buffer == nil {
        return 0
    }
...
func == (lhs: myCRC32, rhs: myCRC32) -> Bool {
    return lhs.initialized == rhs.initialized && lhs.crc == rhs.crc
}

final class myCRC32: Hashable {
    ...
    convenience init(data: NSData) {
        self.init()
        crc = mycrc32(crc, data: data)
        initialized = true
    }
    
    func run(buffer buffer: UnsafePointer<UInt8>, length: Int) {
        crc = mycrc32(crc, buffer: buffer, length: length)
        initialized = true
    }
